...
// SubProductTree 子乘积树的目标是在点值表示的情况下，更高效地计算多项式在一组点上的值。
...
// PolyInterpolation 通过拉格朗日插值法（Lagrange Interpolation）在给定一组点及其对应的函数值的情况下，计算多项式的系数表示
...
// PolyMultiEvaluate 在特定的点上对多项式进行多点求值（multi-point evaluation），并返回求值结果
func PolyMultiEvaluate(f []mcl.Fr, sp *SubProduct) []mcl.Fr {
	...
	if len(sp.Poly) <= 2 {
		ret := make([]mcl.Fr, 1)
		err := mcl.FrEvaluatePolynomial(&ret[0], f, &(sp.Poly[0]))
		if err != nil {
			return nil
		}
		return ret
...
// PolyDifferentiate 对给定的多项式进行求导运算，返回导数的系数表示.
